import pytest
from datetime import datetime, timedelta
from models.temporal_task import TemporalTask
from models.time_interval import TimeInterval

def test_constructor_with_invalid_dates():
    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 1), end_date=datetime(2020, 10, 1))

def test_constructor_with_out_of_scope_lines():
    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 9, 1), end_date=datetime(2025, 10, 2), startline=datetime(2025, 9, 2), deadline=datetime(2025, 10, 2))
        
    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 9, 1), end_date=datetime(2025, 10, 2), startline=datetime(2025, 9, 2))

    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 9, 2), end_date=datetime(2025, 10, 2), deadline=datetime(2025, 10, 1))
    
    reschedules = [TimeInterval(datetime(2025, 10, 1), datetime(2025, 10, 2)), TimeInterval(datetime(2025, 10, 3), datetime(2025, 10, 4))]
    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), deadline=datetime(2025, 10, 9), schedule_intervals=reschedules)
        
    with pytest.raises(ValueError):
        TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2), schedule_intervals=reschedules)

def test_get_start_date():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    assert task.get_start_date() == datetime(2025, 10, 2)

def test_get_end_date():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    assert task.get_end_date() == datetime(2025, 10, 10)
    
def test_get_startline():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2))
    assert task.get_startline() == datetime(2025, 10, 2)

def test_get_total_time():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    assert task.get_total_time() == datetime(2025, 10, 10) - datetime(2025, 10, 2)

def test_get_time_slot():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    assert task.get_time_slot() == TimeInterval(datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))

def test_get_schedule_interval_no_values():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    assert task.get_schedule_intervals() == [TimeInterval(datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))]

def test_get_schedule_interval_with_values():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), schedule_intervals=[TimeInterval(datetime(2025, 10, 3), datetime(2025, 10, 11))])
    assert task.get_schedule_intervals() == [TimeInterval(datetime(2025, 10, 2), datetime(2025, 10, 11))]

def test_get_duration():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), schedule_intervals=[TimeInterval(datetime(2025, 10, 3), datetime(2025, 10, 11))])
    assert task.get_duration() == timedelta(8)

def test_add_schedule_interval():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))
    task.add_schedule_interval(TimeInterval(datetime(2025, 10, 3), datetime(2025, 10, 11)))
    assert task.get_schedule_intervals() == [TimeInterval(datetime(2025, 10, 2), datetime(2025, 10, 11))]
        
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2), deadline=datetime(2025, 10, 10))
    task.add_schedule_interval(TimeInterval(datetime(2025, 10, 2), end_date=datetime(2025, 10, 10)))
    assert task.get_schedule_intervals() == [TimeInterval(datetime(2025, 10, 2), end_date=datetime(2025, 10, 10))]
        
def test_add_schedule_interval_invalid_values():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2), deadline=datetime(2025, 10, 10))
    with pytest.raises(ValueError):
        pytest.raises(ValueError, task.add_schedule_interval(TimeInterval(datetime(2025, 10, 1), end_date=datetime(2025, 10, 10))))
    with pytest.raises(ValueError):
        pytest.raises(ValueError, task.add_schedule_interval(TimeInterval(datetime(2025, 10, 2), datetime(2025, 10, 12))))

def test_to_dict():
    task = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2), deadline=datetime(2025, 10, 10))
    
    expected = {'type': 'TemporalTask', 'title': 'test', 'description': 'this is a test task', 'completed': False, 'deadline': '2025-10-10T00:00:00', 'start_date': '2025-10-02T00:00:00', 'end_date': '2025-10-10T00:00:00', 'startline': '2025-10-02T00:00:00', 'schedule_intervals': [{'start_date': '2025-10-02T00:00:00', 'end_date': '2025-10-10T00:00:00'}]}

    assert task.to_dict() == expected

def test_from_dict():
    json = {'type': 'TemporalTask', 'title': 'test', 'description': 'this is a test task', 'completed': False, 'deadline': '2025-10-10T00:00:00', 'start_date': '2025-10-02T00:00:00', 'end_date': '2025-10-10T00:00:00', 'startline': '2025-10-02T00:00:00', 'schedule_intervals': [{'start_date': '2025-10-02T00:00:00', 'end_date': '2025-10-10T00:00:00'}]}
    task = TemporalTask.from_dict(json)

    expected = TemporalTask(title="test", description="this is a test task", start_date=datetime(2025, 10, 2), end_date=datetime(2025, 10, 10), startline=datetime(2025, 10, 2), deadline=datetime(2025, 10, 10))

    assert task == expected